...
export default function SimulationTab() {
  ...
  const start = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const dumpDuration = Math.max(80, 500 / speed);
  ...
  const loadTruck = useCallback((idx: number) => {
    ...
    setPlan(runLocalEngine(PRESET_POLYS[selectedPreset].poly, truck, 5));
    setCompletedIds(new Set()); setActiveId(null); setCurrentStep(0);
  }, [selectedPreset, stop]);
      ...
      <div className="flex flex-col gap-3 w-64 shrink-0 overflow-y-auto">
        ...
        <div className="bg-card border border-border rounded p-3">
          ...
          {PRESET_POLYS.map((p, i) => (
            <button key={i} onClick={() => loadPreset(i)}
              ...
              data-testid={`sim-preset-${i}`}>{p.name}</button>
          ))}
        </div>
        ...
        <div className="bg-card border border-border rounded p-3">
          ...
          {DEFAULT_TRUCKS.map((t, i) => (
            <button key={t.id} onClick={() => loadTruck(i)}
              className={`block w-full text-left text-xs px-2 py-1.5 rounded border mb-1 ${
                selectedTruckIdx === i ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary hover:bg-muted"
              }`}
              data-testid={`sim-truck-${t.id}`}>{t.name}</button>
          ))}
        </div>
        ...
        <div className="bg-card border border-border rounded p-3">
          ...
          <div className="flex gap-2 mb-3">
            {playing
              ? <button onClick={stop}
                  className="flex-1 py-1.5 text-xs bg-secondary border border-border rounded hover:bg-muted"
                  data-testid="button-pause">Pause</button>
            ...
            <button onClick={reset}
              className="py-1.5 px-3 text-xs bg-secondary border border-border rounded hover:bg-muted"
              data-testid="button-reset">Reset</button>
          </div>
            ...
            {[1, 2, 4].map((s) => (
              <button key={s} onClick={() => setSpeed(s)}
                className={`flex-1 py-1 text-xs rounded border ${
                  speed === s ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary"
                }`}
                data-testid={`speed-${s}x`}>{s}×</button>
            ))}
          </div>

          <div className="text-xs text-muted-foreground mb-1">Progress</div>
          <div className="h-2 bg-secondary rounded overflow-hidden mb-1">
            <motion.div className="h-full bg-green-500 rounded"
        ...
        <div className="flex items-center justify-between px-1 text-xs text-muted-foreground">
          ...
          <div className="flex gap-3 items-center">
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full bg-red-500 inline-block" />Pending
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full bg-white inline-block" />Active
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full bg-green-500 inline-block" />Dumped
            </span>
          </div>
        </div>
